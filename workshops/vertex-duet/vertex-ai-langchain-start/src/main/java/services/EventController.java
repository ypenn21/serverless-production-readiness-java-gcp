...
public class EventController {
  ...
  @PostConstruct
  public void init() {
    logger.info("ImageAnalysisApplication: EventController Post Construct Initializer "
        + new SimpleDateFormat("HH:mm:ss.SSS").format(
        new java.util.Date(System.currentTimeMillis())));
    logger.info(
        "ImageAnalysisApplication: EventController Post Construct - StartupCheck can be enabled");
  ...
  @GetMapping("start")
  String start() {
    logger.info("ImageAnalysisApplication: EventController - Executed start endpoint request "
        + new SimpleDateFormat("HH:mm:ss.SSS").format(
        new java.util.Date(System.currentTimeMillis())));
  ...
  public ResponseEntity<String> receiveMessage(
    ...
    logger.info(
        "EventController: Active processors: " + Runtime.getRuntime().availableProcessors());

    //----------------------------------------------------------------------
    // validate headers, cloud Events headers, file name validation - OOB !
    //----------------------------------------------------------------------
    ...
    //--------------------------------------------------
    // [1] prepare request for Vision API
    // --------------------------------------------------
    /*
    try (ImageAnnotatorClient vision = ImageAnnotatorClient.create()) {
      ...
      */
      //----------------------------------------------------------------------
      // [2] call Vision API, retrieve result
      //----------------------------------------------------------------------
      ...
      labels = response.getLabelAnnotationsList().stream()
      ...
      */
      //----------------------------------------------------------------------
      // [3] extract metadata from Vision response, extract logo, and any text from image
      // is image safe?
      // is image a logo? extract properties
      // are there any text annotations? extract annotations
      //----------------------------------------------------------------------
      ...
      */
      //----------------------------------------------------------------------
      // [4] prepare prompt, call chat-bison@002 model
      //----------------------------------------------------------------------
      String modelResponse = null;
      /*
      StringBuilder prompt = new StringBuilder("Explain the text ");
      String textElements;
      ...
      if (!prompt.isEmpty()) {
        VertexAiChatModel vertexAiChatModel = VertexAiChatModel.builder()
            ...
            .maxRetries(3)
            .build();
        Response<AiMessage> resp = vertexAiChatModel.generate(UserMessage.from(prompt.toString()));
        modelResponse = resp != null ? resp.content().text() : "No response from Chat Model";
        logger.info("Result Chat Model: " + modelResponse);
      }
      */
      //----------------------------------------------------------------------
      // [5] prepare prompt, call text-bison@002 model
      //----------------------------------------------------------------------
      ...
      if (!prompt.isEmpty()) {
        VextexAiLanguageModel vertexAiTextModel = VextexAiLanguageModel.builder()
            ...
            .maxRetries(3)
            .build();
        Response<String> resp = vertexAiTextModel.generate(prompt.toString());
        textResponse = resp != null ? resp.content() : "No response from Text Model";
        logger.info("Result Text Model: " + textResponse);
      }
      */
      //----------------------------------------------------------------------
      // write metadata to Firestore -- OOB
      //----------------------------------------------------------------------

      // Saving result to Firestore
      if (isSafe && modelResponse != null && textResponse != null) {
        ApiFuture<WriteResult> writeResult = eventService.storeImage(fileName, labels, mainColor,
                        modelResponse,
                        textResponse);
